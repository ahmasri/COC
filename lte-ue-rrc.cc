...
LteUeRrc::DoStartCellSelection (uint16_t dlEarfcn)
{
  ...
  m_dlEarfcn = dlEarfcn;
  std::cout<<"In DoStartCellSelection and state is = "<<(int)GetState()<<std::endl;
    ...
    case CONNECTED_NORMALLY:
      ...
      else
      {
    	 // std::cout<<"UE ID = "<<m_rnti<<" In LteUeRrc::DoRecvRrcConnectionReconfiguration, with m_hysteresisHasValue="<<(uint16_t)msg.newHysteresisHasValue<<std::endl;
    	  m_hysteresis = 9999.0; // as it is not assigned yet
  ...
  if (maxRsrpCellId == 0)
    {
      ...
      std::cout<<"UE ID = "<< m_imsi << "Cell search is unable to detect surrounding cell to attach to"<<std::endl;
    }
  else
    {
      ...
      std::cout<<"UE ID = "<< m_imsi << " will synchronize with max power cell Id "<< maxRsrpCellId <<std::endl;
    }
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A3:
      {
        ...
        double ocn = 0.0; // Ocn, the cell specific offset of the neighbour cell (-24dB - 24dB), others saying about cellIndividualOffset -10 dB (late detection) –> 10 dB (early detection)
        ...
        double off = EutranMeasurementMapping::IeValue2ActualA3Offset (reportConfigEutra.a3Offset); //a3-Offset

/* section 5.5.4.4 of 3GPP TS 36.331 version 13.0.0 Release 13
 *
 ...
 *   The smaller the value of this parameter is, the more easily the intra-frequency measurement events are reported.
 * Mp is the measurement result of the PCell/ PSCell, not taking into account any offsets.
 * Ofp  is the frequency specific offset of the frequency of the PCell/ PSCell (i.e. offsetFreqas defined within measObjectEUTRA
 * corresponding to the frequency of the PCell/ PSCell).
 ...
 * Hysis the hysteresis parameter for this event (i.e. hysteresis as defined within reportConfigEUTRA for this event).
 ...
 *
 *  From http://www.sharetechnote.com/html/Handbook_LTE_MultiCell_Measurement_LTE.html
       ...
       if(m_hysteresisHasValue)
        ...
        for (std::map<uint16_t, MeasValues>::iterator storedMeasIt = m_storedMeasValues.begin ();
             ...
             ++storedMeasIt)
          {
        	...
        	if(m_hysteresisHasValue) // if this true it means MLB is on otherwise will be false
        	{
        		off = 0.0;
        	}
              ...
              //  std::cout <<  "--------------------MLB-----with A3 mn = "<<mn<<std::endl;
              //  std::cout <<  "--------------------MLB-----with A3 mp = "<<mp<<std::endl;
        	//}
        	//else
        	//{
                // Inequality A3-1 (Entering condition): Mn + Ofn + Ocn - Hys > Mp + Ofp + Ocp + Off

        		NS_ASSERT((0.0<= hys) && (hys <=15.0));
                 entryCond = mn + ofn + ocn - hys > mp + ofp + ocp + off;

              //   std::cout <<  "-------------------------with A3 mn = "<<mn<<std::endl;
               //  std::cout <<  "-------------------------with A3 mp = "<<mp<<std::endl;
        	//}
                ...
                if (!hasTriggered)
                  {
                	...
                	//std::cout << Simulator::Now().GetSeconds()<<": UE = "<<m_imsi<<std::endl;
                	//std::cout <<  "-------------------------Neigh cell measurements   = "<< neighvalue <<" dB"<<std::endl;
                	//std::cout <<  "-------------------------Serving cell measurements = "<< servingvalue  <<" dB"<<std::endl;
                   ...
                   // This number of trials may lead to fail for eNB with large number of UEs which requires several trials
                   // and the enb is alive
                   if(m_NumHandoverTrials == m_NumofTrails) // this should be checked
                    {
                	   std::cout << Simulator::Now().GetSeconds()<<": UE = "<< m_imsi<<  "---------m_NumHandoverTrials = "<< m_NumHandoverTrials <<std::endl;
                	   concernedCellsEntry.remove(cellId);
                	   eventEntryCondApplicable = false;



                   // m_randomAccessErrorTrace (m_imsi, m_cellId, m_rnti);
                	   m_connectionTimeout.Cancel ();
                	   Simulator::Schedule (MilliSeconds(1000), &LteUeRrc::RLFConnectionTimeout, this);

                    }// end of if(m_NumHandoverTrials == 5)
...
LteUeRrc::RLFConnectionTimeout()
{
	...
	NS_LOG_FUNCTION (this << m_imsi);
	  ...
	  std::cout<<Simulator::Now().GetSeconds()<< ": ConnectionTimeout: UE Id = ("<< m_imsi<<") try to access Cell ID = ("<<m_cellId<<") For ("<<m_NumRLF_RACHOK<<") times"<<std::endl;
					...
					m_state = IDLE_START;
                  	  //I have to comment this part it caused "ConnectionTimeout: UE Id = (87) try to access Cell ID = (5) For (1) times
                  	 // assert failed. cond="m_lcInfoMap.find (lcId) != m_lcInfoMap.end ()", msg="could not find LCID ",
                  	  //file=../src/lte/model/lte-ue-mac.cc, line=509"
                  	 // LeaveConnectedMode();
                      ...
                      std::cout << Simulator::Now().GetSeconds()<<  "----------HO trails expired---------------m_NumHandoverTrials = "<< m_NumHandoverTrials <<std::endl;
                      m_NumHandoverTrials = 0;
}
void
